...
#include<stdlib.h>
#include<string.h>
//#include<windows.h> //substituir do <unistd.h> para que a funcao loading nao de problema, e tambem na funcao loading o usleep pelo sleep(linha 56)
#include <unistd.h>
#include <locale.h>
//===========STRUCTS=====================
struct Livro
{
    ...
    int estante[11];//Define se o livro existe ou nao
};
struct usuario
{
    char nome [31];
    ...
    int atividade;//define se o usuario existe
    int serial;
    
    int livros_cadastrados;//total cadastrado
    int livros_sobra;//vetores de livros disponiveis ate 10
    float melhor_nota;
    char melhorLivro_nota [31];//o que contiver livro e o nome do livro para o usuario saber
    int mais_pag;
...
//AQUI SERA NECESSARIO DIGITAR IGUAL O TEXTO PEDE POR EXEMPLO CASO EU QUERER O USUARIO A VOU TER QUE DIGITAR "Usuario A"
    do
    {
		u->atividade = 0;
...
//ERROS É A CONTAGEM, QUANDO O IF FOR CONCLUIDO ELE IRA CONSIDERAR O ERRO ALEM DO PERMITIDO E FAZER O USUARIO SEGUIR EM FRENTE
            erros = 6;
        }
        ...
        erros++;
//COMO A TENDENCIA E NUNCA ACABAR OS ERROS POR FALHAS ELE FINALIZA O PROGRAMA
        if (erros == 5) {
...
//os whiles sao para nao dar lixo de memoria(se retirar ele pula o que nao deveria)
            while ((lixo = getchar()) != '\n' && lixo != EOF){}
...
//CASO O USUARIO  DECIDIR SELECIONAR UM LIVRO ACIMA OU ABAIXO DO LISTADO ELE IRA CANCELAR E VOLTAR AO MENU
...
//O contagem-- SERVE PARA O USUARIO MODIFICAR INFINITAMENTE, RECEBENDO CONTAGEM APENAS POR SELECIONAR ERRADO.
	   	switch (escolha)
	   	{
	   		case 1:
	   			printf("Digite:");
	   		...
	   		case 7:
	   			...
	   			contagem--;
	   			break;
//contagem RECEBE 10 PARA SAIR DA FUNCAO
	   		case 8:
	   			printf("\nSaindo...\n===Salvo com sucesso===\n");
	   			contagem = 10;
	   			break;
...
{
 	int escolha_usuario, contagem = 1, n_livro;
 	do
 	{
 		printf("\n1.Excluir\n2.Sair\n\nDigite:");
 		...
 		switch (escolha_usuario)
 		{
 			case 1:
 				...
 				n_livro = pesquisa(u);
 //livros_sobra É PARA FUNCAO ESTATISTICAS, APOS EXCLUIR NAS ESTATISTICAS ELE MOSTRA QUE PODE ADICIONAR MAIS LIVROS
 //E PARA EXCLUSAO LOGICA UTILIZO A estante COMO FATOR 0 OU 1
...
//UTILIZA A SITUACAO DA estante PARA DEFINIR SE EXISTE OU NAO LIVRO
...
int main()
{
    ...
    login(&u);
    //Como eu nao posso colocar um valor fixo e comparar ja que nao pode ser todos os livros cadastrados, coloquei valores impossiveis para alguns e assim que constar algum livro ele vai ser substituido.
